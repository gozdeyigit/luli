...
import { saveWordList } from '@/utils/storage';
...
const CreateWordList = () => {
  ...
  const handleSave = () => {
    ...
    const newList = {
      ...
      audioRecordings: useCustomVoice && Object.keys(audioRecordings).length > 0 ? audioRecordings : undefined,
    };

    saveWordList(newList);
    refreshData();
    showSuccess('Word list created successfully! 🎉');
    navigate('/');
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-200 via-pink-200 to-purple-200 dark:from-purple-900 dark:to-blue-900">
        ...
        <Card className="border-4 border-purple-400 shadow-2xl bg-gradient-to-br from-white to-purple-50">
          <CardHeader className="bg-gradient-to-r from-purple-400 to-pink-400 text-white rounded-t-lg">
            ...
            <CardDescription className="text-white text-lg font-semibold">
              Enter exactly 12 awesome words to practice!
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6 pt-6">
            <div className="space-y-2">
              <Label htmlFor="listName" className="text-xl font-bold text-purple-700">List Name</Label>
              <Input
                id="listName"
                ...
                maxLength={100}
                className="text-xl border-4 border-purple-300 focus:border-purple-500 rounded-xl"
              />
            </div>
            ...
            <div className="space-y-4">
              <Label className="text-xl font-bold text-purple-700">Your 12 Words (all required!)</Label>
              <div className="grid grid-cols-1 gap-4">
                {words.map((word, index) => (
                  <div key={index} className="space-y-2 p-4 bg-white rounded-lg border-2 border-blue-200">
                    <div className="flex items-center gap-2">
                      <Label htmlFor={`word-${index}`} className="text-lg font-semibold text-purple-600 min-w-[80px]">
                        Word {index + 1} 📝
                      </Label>
                      <Input
                        id={`word-${index}`}
                        ...
                        maxLength={50}
                        className="text-lg border-2 border-blue-300 focus:border-blue-500 rounded-lg flex-1"
                      />
                    </div>
